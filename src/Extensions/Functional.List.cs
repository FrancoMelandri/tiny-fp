...
public static partial class Functional
{
    ...
    [Pure]
    public static async Task<S> FoldAsync<S, T>(this IEnumerable<T> @this, S state, Func<S, T, Task<S>> folder)
    {
        foreach (var item in @this)
        {
            state = await folder(state, item);
        }
        return state;
    }

    [Pure]
    public static async Task<S> FoldAsync<S, T>(this IEnumerable<T> @this, Task<S> state, Func<S, T, Task<S>> folder)
    {
        var current = await state; 
        foreach (var item in @this)
        {
            current = await folder(current, item);
        }
        return current;
    }
    ...
    [Pure]
    public static Task<T> ReduceAsync<T>(this IEnumerable<T> @this, Func<T, T, Task<T>> reducer)
        => @this.FoldAsync(default(T), reducer);
